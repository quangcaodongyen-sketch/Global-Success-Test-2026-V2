...
import { getLicenseState, LicenseState } from './utils/licenseManager';
import {
  initAuthDatabase,
  getCurrentUser,
  logoutUser,
  ...
  const handleDownloadMatrixExcel = async () => {
    ...
    try {
      const blob = await generateMatrixExcel(
        suite.matrix,
        paper.adminInfo?.schoolName || 'TRƯỜNG THCS ĐỒNG YÊN',
        paper.examType,
      ...
      <footer className="bg-slate-900 text-slate-400 text-xs py-6 border-t border-slate-800 mt-auto">
        <div className="max-w-7xl mx-auto px-4 text-center space-y-2">
          <p className="font-extrabold text-white text-sm">
            © 2026 - 2027 PHẦN MỀM TẠO ĐỀ TIẾNG ANH THCS (ENG-EXAM PRO & GLOBAL SUCCESS TEST)
          </p>
          <p className="text-slate-400">
            Tác giả & Bản quyền:{' '}
            <strong className="text-blue-400">Thầy giáo Đinh Văn Thành – Trường THCS Đồng Yên, tỉnh Tuyên Quang</strong>{' '}
            | ĐT/Zalo:{' '}
          ...
          <p className="text-[11px] text-slate-500">
            Hệ thống biên soạn Đề kiểm tra chuẩn Công văn 7991/BGDĐT, Công văn 5512/BGDĐT & Thể thức văn bản Nghị định 30/2020/NĐ-CP
          </p>
        </div>
      </footer>
    </div>
